...
// How many teams a pool of players wants: the admin's explicit count, or one team per
// target-size group, always rounded UP to an even number so nobody sits a bye week.
// Exported because callers have to know the count BEFORE the build, to work out which
// existing teams they are reusing and how many new ones they need.
export function teamCount(n, { numTeams = null, targetSize = null } = {}) {
  let count = numTeams ? Number(numTeams) : Math.round(n / (targetSize || 10));
...
export function buildTeams(players, opts = {}) {
  ...
  // The caller may hand us the exact team slots to fill — the bracket's existing teams plus
  // however many new ones it needs. Growing a division from four teams to six must not rename
  // Group 9, so when names are supplied they decide both the count and the order.
  const names = Array.isArray(opts.teamNames) && opts.teamNames.length ? opts.teamNames : null;
  const count = names ? names.length : teamCount(players.length, opts);

  const teams = Array.from({ length: count }, (_, i) => ({ name: names ? names[i] : `Team ${i + 1}`, players: [], coaches: [], mSum: 0 }));

  // 1) Seat COACHES FIRST (FR-2.4/2.6).
  //    A coach who already has one of these teams keeps it — that is the whole point of a
  //    rebuild that adds teams rather than replacing them. Everyone left over is spread so
  //    the emptiest bench fills first, which staffs the NEW teams and leaves the rest alone.
  //    With nothing pre-assigned this is plain round-robin, exactly as before.
  const slotOf = new Map(teams.map((t, i) => [t.name, i]));
  const held = new Set();
  if (opts.coachTeam) {
    for (const c of coaches) {
      const i = slotOf.get(opts.coachTeam[c.id]);
      if (i != null) { teams[i].coaches.push(c); held.add(c.id); }
    }
  }
  const heldOn = teams.map((t) => t.coaches.length);   // staff carried over, before any spreading
  const headsOn = (t) => t.coaches.filter(isHead).length;
  const asstsOn = (t) => t.coaches.length - headsOn(t);
  // Fewest of the metric wins; ties go to the smaller bench, then to a team that carried
  // nothing over — so a spare volunteer staffs a brand-new team before being piled onto one
  // that already has its people.
  const fewest = (metric) => {
    let b = 0;
    for (let i = 1; i < count; i++) {
      const rank = (j) => [metric(teams[j]), teams[j].coaches.length, heldOn[j]];
      const ri = rank(i), rb = rank(b);
      for (let k = 0; k < 3; k++) { if (ri[k] < rb[k]) { b = i; break; } if (ri[k] > rb[k]) break; }
    }
    return b;
  };
  const heads = coaches.filter((c) => !held.has(c.id) && isHead(c));
  const assts = coaches.filter((c) => !held.has(c.id) && !isHead(c));
  for (const c of heads) teams[fewest(headsOn)].coaches.push(c);
  for (const c of assts) teams[fewest(asstsOn)].coaches.push(c);
  ...
  // Named slots are already in the caller's order (Group 9, 10, 11, 12, then the new ones).
  if (!names) teams.sort((a, b) => teamNum(a.name) - teamNum(b.name));

...
export function rosterOrder(order = null) {
  ...
  const dv = (p) => {
    const k = String(p.division || "").trim().toLowerCase();
...
export function divisionOptions(seen = []) {
  ...
  const strays = [...new Set((seen || []).map((v) => String(v || "").trim()).filter(Boolean))]
    .filter((v) => !known.has(v.toLowerCase()))
    .sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
  return out.concat(strays);
}
...
export function attendanceWeek({ week, league = null, division = null, team = null } = {}) {
  ...
  for (const r of getRecords("player")) {
    ...
    if (division && (d.division || "") !== division) continue;
    if (team && (d.team || "") !== team) continue;
    const m = marks.get(r.id);
    rows.push({
      id: r.id,
      name: r.name || d.full_name || `#${r.id}`,
      league: d.league || "", division: d.division || "", team: d.team || "",
...
export function movePlayer(id, changes) {
  ...
  if (Object.prototype.hasOwnProperty.call(changes, "team")) after.team = changes.team || "";
...
export function placeUnassignedPlayers({
  ...
  const divOf = (p) => String(p.d.division || "").trim();

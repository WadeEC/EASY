...
// THIS SEASON'S leagues — the client twin of the server's leagueOptions.
//
// Not every league the system has ever had: a typo from two seasons ago has
// nothing to do with the season on screen. Configured leagues that somebody in
// view is actually in, falling back to all configured ones when the season is
// empty (otherwise there'd be nothing to route the first import into), plus
// any league on a record here that isn't configured, so nobody is stranded.
//
// `seen` must be the leagues of the players ALREADY SCOPED to the season —
// passing an unscoped list puts other seasons' leagues back in the picker.
export function leagueChoices(fields, seen = []) {
  let configured = [];
  try {
    const f = (fields || []).find((x) => x.name === "league");
    const raw = f && f.options ? (typeof f.options === "string" ? JSON.parse(f.options) : f.options) : [];
    configured = (raw || []).map((l) => String(l)).filter(Boolean);
  } catch { configured = []; }

  const inSeason = [...new Set((seen || []).map((x) => String(x || "").trim()).filter(Boolean))].sort();
  const inSeasonKeys = new Set(inSeason.map((l) => l.toLowerCase()));

  let out = configured.filter((l) => inSeasonKeys.has(l.toLowerCase()));
  if (!out.length && !inSeason.length) out = configured.slice();

  const known = new Set(out.map((l) => l.toLowerCase()));
  for (const v of inSeason) if (!known.has(v.toLowerCase())) out.push(v);
  return out;
}

// Put a week-count change into a sentence. Used by the confirmation on both
// the Team Board and Attendance so they can't drift apart.
export function weekCountSummary(pv) {
  if (!pv || pv.error) return { title: "", lines: [] };
  const lines = [];
  if (pv.unchanged) return { title: `Already ${pv.to} weeks.`, lines: [] };

  const title = `Change the season from ${pv.from} to ${pv.to} week${pv.to === 1 ? "" : "s"}?`;
  if (pv.to > pv.from) {
    lines.push(`Adds ${pv.to - pv.from} more week${pv.to - pv.from === 1 ? "" : "s"} to the end.`);
  }
  const gone = (pv.removing || []).length;
  if (gone) {
    lines.push(`Removes ${gone} week${gone === 1 ? "" : "s"}: ${(pv.removing || []).map((w) => w.label).join(", ")}.`);
  }
  const kept = pv.removing_with_checkins || [];
  if (kept.length) {
    lines.push(`${kept.join(", ")} ${kept.length === 1 ? "has" : "have"} check-ins — ${kept.length === 1 ? "it stays" : "they stay"} on the grid as extra. Nothing is deleted.`);
  }
  if ((pv.returning || []).length) {
    lines.push(`${pv.returning.join(", ")} comes back into the season proper.`);
  }
  return { title, lines };
}

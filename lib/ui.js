...
//
// `seen` is the set of division values actually on records right now. Anything
// in there that ISN'T a defined division still gets offered — otherwise the
// players carrying it become unreachable — but it's labelled so it's obvious
...
// [{ value, label, stray }] ready to render as <option>s.
export function divisionChoices(divisions, league, seen = []) {
  const real = divisionsForLeague(divisions, league);
  const known = new Set(real.map((d) => String(d.name || "").trim().toLowerCase()));
  const out = real.map((d) => ({
    value: d.name,
    label: `${d.name}${d.age_min != null && d.age_min !== "" ? ` (${d.age_min}–${d.age_max ?? ""})` : ""}`,
    stray: false,
  }));
  const extras = [...new Set((seen || []).map((v) => String(v || "").trim()).filter(Boolean))]
    .filter((v) => !known.has(v.toLowerCase()))
    .sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
  for (const v of extras) out.push({ value: v, label: `${v} — not a division`, stray: true });
  return out;
}

...
import {
  ...
  attendanceWeek, attendanceWeeks, isPresentRow, rosterOrder,
} from "./tools.js";
...
function playersSheet(players, title = "Roster") {
  ...
  const rows = players.map((p) => [
    p.id, p.name, p.age ?? "", p.league || "", p.second_league || "", p.division || "",
...
function unassignedSheet(u, league) {
  ...
  const push = (label, list) => {
    for (const p of list) {
      ...
      rows.push([label, p.id, p.name, p.age ?? "", p.league || "", p.division || "", p.team || "", p.township || ""]);
    }
  };
...
export function attendanceGridSheet(season, league = null) {
  const weeks = attendanceWeeks(league);
  ...
  const header = ["Player", "Team", "Division", "League", ...weeks.map((w, i) => `Wk ${i + 1} (${w})`), "Present", "Absent"];

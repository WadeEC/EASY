...
// The sidebar's season picker — sent with every request so the server scopes
// players / teams / schedule / standings to the chosen season. "" = all seasons.
function seasonHeader() {
  try {
    const s = typeof localStorage !== "undefined" ? localStorage.getItem("ff_season") : null;
    return s ? { "x-ff-season": s } : {};
  } catch { return {}; }
}
...
export const api = {
  ...
  seasonStart: (name, leagues) => jbody("/api/seasons", { action: "start", name, leagues }),

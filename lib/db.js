...
export function getDb() {
  if (!_db) {
    ...
    _db.exec(SCHEMA);
    // ---- Migrations -------------------------------------------------------
    // ALTER TABLE only succeeds the first time; subsequent calls error. We
    // wrap each ADD COLUMN in try/catch so the app boots cleanly on both
    // fresh installs and existing databases.
    try { _db.exec("ALTER TABLE users ADD COLUMN password_reset_requested INTEGER DEFAULT 0"); } catch {}
    try { _db.exec("ALTER TABLE users ADD COLUMN must_change_password INTEGER DEFAULT 0"); } catch {}
  }
  return _db;
}

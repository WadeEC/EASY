...
//
// A week is STORED as the ISO date of its Sunday — that's the key attendance
// hangs off and it must never change. But nobody running a Saturday league
// thinks of week 2 as "Aug 16"; they think of it as Week 2, played on Saturday
// the 22nd. So the key stays the Sunday, and the label is a name plus the day
// you actually play.
//
// Where weeks come from, in order:
//   1. The schedule — every game's date, snapped back to its Sunday.
//   2. Attendance already taken, for a season with no schedule saved.
//   3. Nothing yet? Generate forward from this week. A league that hasn't
//      built its schedule still has to take attendance on Saturday, and "there
//      are no weeks" is a useless answer to that.
//
// NUMBERING COUNTS WEEKS THAT HAPPEN. If week 2 is rained out, the week after
// it becomes week 2 — the season didn't gain a week, it lost a Saturday. So a
// cancelled week takes no number and everything after it shifts down. A week
// counts as cancelled when its play date is a blackout, or when someone marked
// it cancelled here.
//
// And every label is editable, because the automatic answer is sometimes just
// wrong: leagues have a Jamboree, a Week 0, Picture Day, Playoffs.
const WEEK_META_KEY = "week_meta";   // { "YYYY-MM-DD": { label?, cancelled? } }
...
// WHICH DAY DOES THIS LEAGUE PLAY?
//
// A week with games knows its own date. A week we generated because nothing is
// scheduled yet does not — and showing its Sunday key is how the picker said
// "Sun, Aug 16" on Saturday the 22nd. The Sunday is a filing system, not a
// game day.
//
// Best evidence first:
//   1. The day most games in this league are actually on.
//   2. The league's name — "Saturday Limerick" plays Saturday.
//   3. Saturday, because that's when youth flag football is played.
const _DAY_WORDS = ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"];

function _playDayFor(league = null) {
  ...
  for (const r of getRecords("game")) {
    ...
    if (dow >= 0 && dow <= 6) tally[dow]++;
  }
  const best = tally.indexOf(Math.max(...tally));
  if (tally[best] > 0) return best;

  // Nothing scheduled — read the league names. One league: use its day. Several
  // that disagree: no single answer, so fall through to Saturday.
  const names = league ? [String(league)] : leagueNames();
  const found = new Set();
  for (const n of names) {
    const low = String(n).toLowerCase();
    for (let i = 0; i < 7; i++) if (low.includes(_DAY_WORDS[i])) found.add(i);
  }
  if (found.size === 1) return [...found][0];
  return 6;   // Saturday
}

// How many weeks to lay out when there's no schedule at all.
const PLANNED_WEEKS = 10;

export function seasonWeekList({ league = null, generate = PLANNED_WEEKS } = {}) {
  const play = new Map();            // week (Sunday) -> earliest game date in it
  for (const r of getRecords("game")) {
    let d = {}; try { d = JSON.parse(r.data || "{}"); } catch { continue; }
    if (league && (d.league || "") !== league) continue;
    ...
    if (!play.has(w) || (day && day < play.get(w))) play.set(w, day || "");
  }

  const weeks = new Map(play);
  if (!weeks.size) {
    for (const r of getRecords("attendance")) {
      let d = {}; try { d = JSON.parse(r.data || "{}"); } catch { continue; }
      if (d.week) weeks.set(String(d.week), "");
    }
  }

  const planned = new Set();
  if (!weeks.size && generate > 0) {
    let w = _thisWeekISO();
    for (let i = 0; i < generate; i++) { weeks.set(w, ""); planned.add(w); w = _addDays(w, 7); }
  }

  const now = _thisWeekISO();
  if (!weeks.has(now)) { weeks.set(now, ""); planned.add(now); }

  const meta = _weekMeta();
  let blackout = new Set();
  try { blackout = blackoutDateSet(league); } catch {}
  const playDay = _playDayFor(league);   // 0 = Sunday … 6 = Saturday

  let n = 0;
  return [...weeks.keys()].sort().map((week) => {
    // A week with games knows its date. One we generated gets the league's
    // play day within that week, so a Saturday league reads "Sat, Aug 22"
    // rather than the Sunday it happens to be filed under.
    const date = weeks.get(week) || _addDays(week, playDay);
    const m = meta[week] || {};
    // Blacked-out play date, or cancelled by hand.
    const cancelled = !!m.cancelled || blackout.has(date) || blackout.has(week);
    if (!cancelled) n++;                      // only weeks that happen take a number
    return {
      week,                                   // the Sunday — the storage key
      date,                                   // the day games are actually on
      index: cancelled ? null : n,
      label: m.label || (cancelled ? "Cancelled" : `Week ${n}`),
      named: !!m.label,
      cancelled,
      planned: planned.has(week),             // no games, no attendance — a placeholder
      current: week === now,
    };
  });
}
...
export function setWeekLabel(week, label) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(w)) return { error: "Which week? Give the Sunday as YYYY-MM-DD." };
  const map = _weekMeta();
...
// Cancel a week — it keeps its date but gives up its number, and every week

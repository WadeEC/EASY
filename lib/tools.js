...
// when two exist.
//
// A FILTER should never hide a league — an empty league is a legitimate answer
// ("nobody in Sunday Upper Merion this week"), and hiding it makes the app look
// like it lost one. So pass seasonLeagues only where the list is a DESTINATION
// (moving players into a league), not where it's a filter.
//
// Any league actually sitting on a record is included too: unlike a division,
// a league isn't derived from anything, so dropping a stray would strand the
// players carrying it.
export function leagueOptions(seen = [], seasonLeagues = null) {
  const f = getFields("player").find((x) => x.name === "league");
  let opts = []; try { opts = f && f.options ? JSON.parse(f.options) : []; } catch { opts = []; }
  if (Array.isArray(seasonLeagues) && seasonLeagues.length) {
    opts = opts.filter((l) => seasonLeagues.includes(l));
  }

...
export function scheduleTeams(league = null) {
  const set = new Set();
  for (const r of getRecords("player")) {
    let d = {}; try { d = JSON.parse(r.data || "{}"); } catch {}
    if (!d.team) continue;
...
export function getSchedule(league = null) {
  const out = [];
...
export function saveSchedule(league = null, games = []) {
  seedGamesSection();
  for (const r of getRecords("game")) {
    let d = {}; try { d = JSON.parse(r.data || "{}"); } catch {}
    if ((d.league || "") === (league || "")) deleteRecord(r.id, "user");
  }
  ...
  for (const g of games || []) {
    ...
    createRecord("game", { week: g.week, date: g.date || "", time: g.time || "", league: league || "", home_team: g.home, away_team: g.away, location: g.location || "", referee: g.referee || "" }, `Week ${g.week}: ${g.home} vs ${g.away}`);
    saved++;
  }
  return { saved, dropped_cross_division: droppedCrossDivision };
}
...
export function generateSchedule(league = null, opts = {}) {
  ...
  const res = saveSchedule(league, games);
  return {
    status: "created",
    league: league || "",
    teams: teams.length,
...
export function getStandings(league = null) {

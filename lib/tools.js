...
import { buildSchedule, weekDate, clockTime, placeOnFields, minutesOf as _minutesOf } from "./schedule.js";
...
// ---------------------------------------------------------------- whitespace hygiene
// Spreadsheets carry stray spacing: "Connor  Johnson", "Ages 13-14 ", non-breaking
// spaces pasted from a web page. Pages that scope by an EXACT match — the Team
// Editor's league + division filter — then silently drop those records: they exist,
// they just never equal the selected value. (isKnownDivision normalizes spacing when
// it COMPARES, so a padded division passes validation and is then stored padded.)
// So every write normalizes strings on the way in:
//   - non-breaking spaces become ordinary spaces
//   - both ends are trimmed
//   - runs of spaces/tabs inside a SINGLE-LINE value collapse to one space
// Multi-line values keep their line breaks; prose fields keep their interiors; and
// anything that parses as JSON (a tournament bracket, rank history) is left alone.
export function tidyText(v, { collapse = true } = {}) {
  if (typeof v !== "string") return v;
  const t = v.replace(/[\u00a0\u2007\u202f]/g, " ").trim();
  if (!collapse || /[\r\n]/.test(t)) return t;
  if (/^[{[]/.test(t)) { try { JSON.parse(t); return t; } catch {} }
  return t.replace(/[ \t]+/g, " ");
}
// Prose fields are things a person typed as sentences, not values anything matches
// on. Their ends still get trimmed, but double spaces inside them are left alone.
const PROSE_FIELD = /(^|_)(notes?|reason|description|comment|memo|message|address)(_|$)/i;
export function tidyData(obj) {
  if (!obj || typeof obj !== "object" || Array.isArray(obj)) return obj;
  const out = {};
  for (const [k, v] of Object.entries(obj)) out[k] = tidyText(v, { collapse: !PROSE_FIELD.test(k) });
  return out;
}
...
export function applyCreateRecord(rtype, rname, data, actor) {
  // Normalize spacing FIRST so assignment rules, league resolution, the division
  // registry check and season stamping all compare clean values — an imported
  // "Ages 13-14 " must not be stored with its trailing space.
  rname = tidyText(rname);
  data = tidyData(data);
...
export function updateRecord(id, fields = {}, actor = "user", reason = "updated record") {
  ...
  const patch = tidyData(fields);
  const merged = { ...JSON.parse(before.data || "{}"), ...patch };
  ...
  if (owned) {
    if (Object.prototype.hasOwnProperty.call(patch, "season")) {
      season = hasValue(patch.season) ? String(patch.season).trim() : null;
      const blocked = assertWritable(season);
      if (blocked) return { error: blocked };
    }
    if (season) merged.season = season; else delete merged.season;
  }
  // The display name lives in its own column (records.name), and every screen reads
  // that column FIRST — coach pills, check-in rows, the coach-to-player links. A
  // rename used to write only data.full_name, so the old name kept showing up all
  // over the app. Keep the column in step whenever a rename comes through.
  const renamedTo = ("full_name" in patch || "name" in patch) ? tidyText(patch.full_name ?? patch.name) : null;
  const nextName = renamedTo || before.name;
  const auditReason = (nextName !== before.name && reason === "updated record") ? `renamed to ${nextName}` : reason;
  getDb().prepare("UPDATE records SET name=?, data=?, season=?, updated_at=? WHERE id=?")
    .run(nextName, JSON.stringify(merged), owned ? (season || null) : before.season, now(), id);
  logAudit(actor, "update", "records", id, before, getRow("records", id), auditReason);
  return { status: "updated", id, ...(nextName !== before.name ? { renamed: nextName } : {}) };
}

// One-time repair for data written before the normalization above (or pasted in by
// hand): walk every record, tidy the display name and every text value, and report
// exactly what changed. Each write is audited, so the Change Log lists it and Time
// Machine can revert the batch. Records in a LOCKED season are left alone — a
// finished season stays exactly as it was. dryRun = preview only.
export function tidyWhitespace({ dryRun = false, type = null, actor = "user(cleanup)" } = {}) {
  const d = getDb();
  const rows = type
    ? d.prepare("SELECT * FROM records WHERE type=? ORDER BY id").all(slug(type))
    : d.prepare("SELECT * FROM records ORDER BY id").all();
  const changes = [];
  const locked = [];
  for (const r of rows) {
    let data = {};
    try { data = JSON.parse(r.data || "{}"); } catch { continue; }
    const tidied = tidyData(data);
    const newName = tidyText(r.name);
    const fieldsChanged = Object.keys(tidied).filter((k) => tidied[k] !== data[k]);
    const nameChanged = (newName == null ? null : newName) !== (r.name == null ? null : r.name);
    if (!nameChanged && !fieldsChanged.length) continue;
    if (isSeasonOwned(r.type) && assertWritable(r.season)) { locked.push({ id: r.id, season: r.season }); continue; }
    changes.push({
      id: r.id,
      type: r.type,
      name: newName || r.name || `#${r.id}`,
      fields: [...(nameChanged ? ["name"] : []), ...fieldsChanged],
      before: [...(nameChanged ? [`name: "${r.name}"`] : []), ...fieldsChanged.map((k) => `${k}: "${data[k]}"`)].join(" · "),
      after: [...(nameChanged ? [`name: "${newName}"`] : []), ...fieldsChanged.map((k) => `${k}: "${tidied[k]}"`)].join(" · "),
    });
    if (dryRun) continue;
    d.prepare("UPDATE records SET name=?, data=?, updated_at=? WHERE id=?")
      .run(newName, JSON.stringify(tidied), now(), r.id);
    logAudit(actor, "update", "records", r.id, r, getRow("records", r.id), "tidied extra spaces");
  }
  return {
    status: dryRun ? "preview" : "cleaned",
    scanned: rows.length,
...
// Companion repair for renames made before updateRecord kept the two in step: any
// record whose data.full_name (or data.name, for tournaments and divisions) has
// drifted from its display name gets the column refreshed. Locked seasons are
// skipped, same as above. dryRun = preview only.
export function resyncDisplayNames({ dryRun = false, type = null, actor = "user(cleanup)" } = {}) {
  const d = getDb();
  const rows = type
    ? d.prepare("SELECT * FROM records WHERE type=? ORDER BY id").all(slug(type))
    : d.prepare("SELECT * FROM records ORDER BY id").all();
  ...
  for (const r of rows) {
    let data = {};
    try { data = JSON.parse(r.data || "{}"); } catch { continue; }
    const want = tidyText(data.full_name ?? data.name);
    // Only records that carry their own name field are in scope — people use
    // full_name, tournaments and divisions use name. A game's display name
    // ("Week 3: A vs B") is generated, not stored, so games are skipped.
    if (!want || want === r.name) continue;
    if (isSeasonOwned(r.type) && assertWritable(r.season)) { locked.push({ id: r.id, season: r.season }); continue; }
    changes.push({ id: r.id, type: r.type, name: want, fields: ["name"], before: `name: "${r.name}"`, after: `name: "${want}"` });
    if (dryRun) continue;
    d.prepare("UPDATE records SET name=?, updated_at=? WHERE id=?").run(want, now(), r.id);
    logAudit(actor, "update", "records", r.id, r, getRow("records", r.id), "re-synced display name after rename");
  }
  return {
    status: dryRun ? "preview" : "cleaned",
    scanned: rows.length,
...
// Does this bracket's age range actually contain this player?
export function ageFitsDivision(divisionName, data) {
  const dv = getDivisions().find((d) => String(d.name).trim().toLowerCase() === String(divisionName || "").trim().toLowerCase());
  if (!dv) return null;                                   // not a real bracket
  const age = Number(data && data.age);
  if (!Number.isFinite(age) || String((data && data.age) ?? "").trim() === "") return null;   // no age to judge
  const min = dv.age_min === "" || dv.age_min == null ? -Infinity : Number(dv.age_min);
  const max = dv.age_max === "" || dv.age_max == null ? Infinity : Number(dv.age_max);
  return age >= min && age <= max;
}

// AGE OVERRIDE — a player deliberately placed outside their age bracket.
//
// Real leagues need this. Two thirteen-year-olds move to a league whose oldest
// bracket is 10-12: there is nowhere else for them to play, so they play up or
// down and that is a decision someone made, not a mistake to correct.
//
// The flag exists because "re-sort into brackets" would otherwise undo it
// every time it ran, silently, and nobody would know until a parent asked why
// their kid was moved back. A player with division_override is left alone by
// every automatic sort — and only by an automatic sort. Moving them by hand
// still works.
export function isDivisionOverridden(data) { return (data && data.division_override) === true; }
...
export function divisionOf(data) {
  ...
  // An age override is the whole point: they are here on purpose.
  if (stored && isDivisionOverridden(data) && isKnownDivision(stored)) return stored;
...
export function reassignDivisions() {
  ...
  for (const r of getRecords("player")) {
    ...
    // Age overrides are decisions. An automatic sort does not get to undo one.
    if (isDivisionOverridden(data)) {
      kept.push({ id: r.id, name: r.name || data.full_name || `#${r.id}`, division: data.division || "", age: data.age ?? null });
      continue;
    }
  ...
  return {
    updated,
    kept: kept.length,
    kept_details: kept.slice(0, 20),
    note: kept.length ? `${kept.length} player(s) have an age override and were left where they are.` : undefined,
  };
}

// Turn the override on or off directly, for one player.
export function setDivisionOverride(id, on = true, reason = "") {
  const row = getRow("records", Number(id));
  if (!row || row.type !== "player") return { error: "Player not found." };
  let d = {}; try { d = JSON.parse(row.data || "{}"); } catch {}
  const res = updateRecord(Number(id), { division_override: !!on },
    "user(age override)", reason || (on ? "age override on" : "age override off"));
  if (res.error) return res;
  return {
    ok: true, id: Number(id), name: row.name, division: d.division || "", age: d.age ?? null,
    division_override: !!on,
    note: on
      ? "The automatic sort will leave them in this bracket."
      : "They'll sort by age again the next time divisions are re-sorted.",
  };
}
...
export function getCoaches(league = null) {
  ...
  return getRecords("coach").map((r) => {
    let d = {}; try { d = JSON.parse(r.data || "{}"); } catch {}
    return { id: r.id, name: r.name || d.full_name || `#${r.id}`, role: d.role || "", league: d.league || "", child: d.child_name || "", team: d.team || "" };
  }).filter((c) => !league || !c.league || c.league === league);
}

// ---------------------------------------------------------------- coaches by bracket
//
// A coach record has no division field, so which bracket a coach belongs to has to be
// worked out. Without this, building ONE division hands the builder every coach in the
// league, spreads all of them across that bracket's teams, and saving then rewrites the
// `team` of coaches who were never in scope — stripping them off their real teams league-wide.
//
// The order of evidence matters:
//   1) their child's bracket — coach_child pinning means the coach follows the kid, so the
//      kid's age is the strongest signal there is.
//   2) the bracket of the team they're on now, taken from who actually plays on it.
//   3) the division prefix on that team's name ("Ages 13-14 / Group 9"), for a team whose
//      players haven't been assigned yet.
// A coach with none of the three is UNASSIGNED: no team, no child, free to take a new team.
// Returns Map(coach id -> division name), with unassigned coaches simply absent.
export function coachDivisionMap(league = null, season = null) {
  const coaches = getCoaches(league);
  if (!coaches.length) return new Map();

  // Player name -> bracket, for the child lookup.
  const divByPlayer = new Map();
  for (const r of getRecords("player")) {
    let d = {}; try { d = JSON.parse(r.data || "{}"); } catch {}
    if (league && (d.league || "") !== league && (d.second_league || "") !== league) continue;
    const key = String(r.name || d.full_name || "").trim().toLowerCase();
    if (key) divByPlayer.set(key, divisionOf(d));
  }
  const teamDiv = teamDivisionMap(league, season);
  // Same splitter the builder uses — one coach may list two or three kids.
  const splitKids = (x) => String(x || "").split(/\s*(?:,|;|\/|\band\b|&)\s*/i).map((k) => k.trim().toLowerCase()).filter(Boolean);

  const out = new Map();
  for (const c of coaches) {
    let dv = "";
    for (const kid of splitKids(c.child)) { const d = divByPlayer.get(kid); if (d) { dv = d; break; } }
    if (!dv && c.team) {
      const info = teamDiv.get(String(c.team));
      dv = (info && info.division) || _divisionOfTeam(c.team);
    }
    if (dv) out.set(c.id, dv);
  }
  return out;
}

// The coaches a build of ONE bracket is allowed to touch: the ones already in it, plus the
// spare volunteers who belong nowhere yet and can take a newly created team. Coaches tied to
// another bracket are never returned, so their assignments survive the build untouched.
export function coachesForDivision(league = null, division = null, { includeUnassigned = true, season = null } = {}) {
  const all = getCoaches(league);
  if (!division) return all;            // no bracket asked for — unchanged behaviour
  const map = coachDivisionMap(league, season);
  return all.filter((c) => {
    const dv = map.get(c.id);
    return dv ? dv === division : includeUnassigned;
  });
}

// The team slots a bracket's rebuild should fill.
//
// Changing a division from four teams to six should ADD two teams, not rename the four that
// already exist and re-shuffle their staff. So the existing teams come back first, in play
// order, each with the coaches already on it; only the shortfall is newly named, continuing
// whatever numbering is already in use ("… / Group 12" -> "… / Group 13") and skipping any
// name taken anywhere in the league.
//
// Returns { names, coachTeam, kept, added, dropped } — dropped is non-empty only when the
// admin asked for FEWER teams than the bracket has, and those teams' players and coaches get
// folded back into the ones that remain.
export function divisionTeamSlots(league = null, division = null, count = 0, season = null) {
  const empty = { names: [], coachTeam: {}, kept: [], added: [], dropped: [] };
  if (!count) return empty;

  const teamDiv = teamDivisionMap(league, season);
  const existing = [...teamDiv.entries()]
  ...
  // Every team name in use anywhere, so a new one can't land on top of another bracket's.
  const taken = new Set([...teamDiv.keys()].map(String));
  for (const c of getCoaches(null)) if (c.team) taken.add(String(c.team));

  const added = [];
  const need = count - kept.length;
  if (need > 0) {
    // Follow the numbering already in use; fall back to the bracket-prefixed default.
    const m = kept.length ? String(kept[kept.length - 1]).match(/^(.*?)(\d+)\s*$/) : null;
    const stem = m ? m[1] : (division ? `${division} / Team ` : "Team ");
    let n = m ? Number(m[2]) : 0;
    let guard = 0;
    while (added.length < need && guard++ < 500) {
      n++;
      const nm = `${stem}${n}`;
      if (!taken.has(nm) && !added.includes(nm)) added.push(nm);
    }
  }

  const names = [...kept, ...added];
  const slots = new Set(names);
  const coachTeam = {};
  for (const c of getCoaches(league)) if (c.team && slots.has(String(c.team))) coachTeam[c.id] = String(c.team);
  return { names, coachTeam, kept, added, dropped };
}
...
export async function buildAndSaveTeams({ league = null, division = null, target_size = null, num_teams = null, perDivision = false } = {}) {
  ...
  const { buildTeams, teamCount } = await import("./teams.js");
  ...
  // A spare coach can only be handed to one bracket, however many we build in this pass.
  const spent = new Set();

  for (const slice of slices) {
    if (!slice.players.length) continue;
    const dv = slice.division && slice.division !== "(no division)" ? slice.division : null;
    // Scoped to the bracket: a build of one division must never rewrite another's coaches.
    const coaches = coachesForDivision(slice.league || null, dv).filter((c) => !spent.has(c.id));
    const count = teamCount(slice.players.length, { numTeams: num_teams, targetSize: target_size });
    const slots = dv ? divisionTeamSlots(slice.league || null, dv, count) : null;
    const result = buildTeams(slice.players, {
      ...
      teamNames: slots ? slots.names : null,     // keep the bracket's existing teams
      coachTeam: slots ? slots.coachTeam : null, // and the staff already on them
      rules, coaches, coachChild, links,
    });
    const teams = result.teams || [];

    // Commit: prefix team names with the division when slicing per-division, so
    // "Team 1" doesn't collide across brackets (e.g. "9-10 / Team 1"). Reused slot
    // names already carry their bracket, so they are written through as-is.
    const named = !!(slots && slots.names.length);
    const prefix = !named && dv ? `${dv} / ` : "";
    for (const t of teams) {
      ...
      if (hasCoachSection) {
        for (const c of (t.coaches || [])) {
          spent.add(c.id);
          if (String(c.team || "") === teamName) continue;   // already there — don't churn the log
    ...
    built.push({
      ...
      kept_teams: slots ? slots.kept : [], new_teams: slots ? slots.added : [], dropped_teams: slots ? slots.dropped : [],
    });
  }
...
// ---------------------------------------------------------------- rename a team
// A team has no record of its own: its name is a STRING copied onto every player,
// every coach, both sides of every scheduled game, and any tournament bracket it
// was seeded into. So a rename is a cascade, not an edit — miss one of those and
// the schedule stops matching the roster, which is exactly the "record exists on
// one screen but not another" failure this app has been bitten by before.
//
// Everything here goes through updateRecord, so each write lands in the Change Log
// and Time Machine can revert the whole batch.
export function renameTeam(oldName, newName, { league = null, actor = "user(rename)" } = {}) {
  const from = tidyText(String(oldName ?? ""));
  const to = tidyText(String(newName ?? ""));
  if (!from) return { error: "No team to rename." };
  if (!to) return { error: "A team needs a name." };
  if (to === from) return { ok: true, unchanged: true, from, to };
  if (to.length > 80) return { error: "That team name is too long (80 characters max)." };

  const sameTeam = (v) => tidyText(String(v ?? "")) === from;

  const players = getRecords("player").map((r) => ({ r, d: _pdata(r) }))
    .filter(({ d }) => sameTeam(d.team) && (!league || (d.league || "") === league));
  const hasCoachSection = getRecordTypes().some((t) => t.name === "coach");
  const coaches = hasCoachSection
    ? getRecords("coach").map((r) => ({ r, d: _pdata(r) })).filter(({ d }) => sameTeam(d.team))
    : [];
  const hasGameSection = getRecordTypes().some((t) => t.name === "game");
  const games = hasGameSection
    ? getRecords("game").map((r) => ({ r, d: _pdata(r) }))
      .filter(({ d }) => sameTeam(d.home_team) || sameTeam(d.away_team))
    : [];

  if (!players.length && !coaches.length && !games.length) {
    return { error: `No team called "${from}" in this season.` };
  }

  // Renaming onto a name already in use would silently MERGE two rosters. That's a
  // different operation with different consequences, so refuse and say so.
  const taken = scheduleTeams(league).some((t) => tidyText(String(t)) === to);
  if (taken) return { error: `A team called "${to}" already exists. Pick a different name, or move the players instead of renaming.` };

  // Team names carry their bracket as a "Division / Team" prefix, and movePlayer
  // enforces that prefix. A name whose prefix contradicts the players on the team
  // would make every later move of those players fail, so catch it now.
  if (to.includes("/")) {
    const prefix = to.split("/")[0].trim();
    const info = teamDivisionMap(league).get(from);
    if (prefix && info && !info.mixed && info.division && prefix !== info.division) {
      return { error: `"${to}" says ${prefix}, but this team's players are in ${info.division}. Use "${info.division} / ..." or move the players first.` };
    }
  }

  // A locked or archived season is read-only — check before writing anything, so a
  // rename is all-or-nothing rather than half-applied.
  for (const { r } of [...players, ...coaches, ...games]) {
    const blocked = assertWritable(r.season);
    if (blocked) return { error: blocked };
  }

  const reason = `team renamed: ${from} → ${to}`;
  let playersChanged = 0, coachesChanged = 0, gamesChanged = 0;
  for (const { r } of players) { const res = updateRecord(r.id, { team: to }, actor, reason); if (!res.error) playersChanged++; }
  for (const { r } of coaches) { const res = updateRecord(r.id, { team: to }, actor, reason); if (!res.error) coachesChanged++; }
  for (const { r, d } of games) {
    const patch = {};
    if (sameTeam(d.home_team)) patch.home_team = to;
    if (sameTeam(d.away_team)) patch.away_team = to;
    const res = updateRecord(r.id, patch, actor, reason);
    if (!res.error) gamesChanged++;
  }

  // Brackets keep their own copy of the seeded names (plus the winner recorded on
  // each match), so rewrite those too — otherwise a renamed team shows up twice.
  let bracketsChanged = 0;
  if (getRecordTypes().some((t) => t.name === "tournament")) {
    for (const r of getRecords("tournament")) {
      const d = _pdata(r);
      if (!d.state) continue;
      let st; try { st = JSON.parse(d.state); } catch { continue; }
      let touched = false;
      const swap = (v) => { if (sameTeam(v)) { touched = true; return to; } return v; };
      if (Array.isArray(st.teams)) st.teams = st.teams.map(swap);
      if (Array.isArray(st.rounds)) for (const round of st.rounds) for (const m of round || []) {
        if (!m) continue;
        m.home = swap(m.home); m.away = swap(m.away);
        if (m.winner) m.winner = swap(m.winner);
      }
      if (!touched) continue;
      const res = updateRecord(r.id, { state: JSON.stringify(st) }, actor, reason);
      if (!res.error) bracketsChanged++;
    }
  }

  return { ok: true, from, to, players: playersChanged, coaches: coachesChanged, games: gamesChanged, brackets: bracketsChanged };
}
...
// Minutes past midnight for a saved game time — "1:00 PM" or "13:00". null when unreadable.
function _gameMinutes(t) {
  const m = /^\s*(\d{1,2}):(\d{2})\s*(AM|PM)?/i.exec(String(t || ""));
  if (!m) return null;
  let h = +m[1];
...
const _naturally = (a, b) => String(a).localeCompare(String(b), undefined, { numeric: true });

// Repair for schedules built before divisions shared a field ledger: two brackets
// that kick off at the same hour both started at Field 1, so the saved schedule
// has two games on one field at one time.
//
// This re-lays ONLY the field each game is on. Dates, times, matchups, referees
// and scores are left exactly as they are, so a season already underway keeps its
// assignments — unlike a rebuild, which replaces every game.
//
// Who keeps their fields: the division that starts earliest goes first and holds
// what it has; later divisions move up to the next free field (Ages 7-8 keeps
// Fields 1-2 at 1:00, Ages 9-10 shifts to 3, 4, 5). A game only moves if the field
// it is on is already taken at its time.
//
// The field pool is whatever that league's schedule already uses. `fieldsCount`
// raises it — say 5 when you have five fields but the schedule only ever used
// three, and the extra "Field 4" / "Field 5" become available to move games onto.
// Games with nowhere to go are reported as `unresolved` rather than stacked up.
export function fixFieldClashes({ dryRun = false, league = null, fieldsCount = null, actor = "user(cleanup)" } = {}) {
  const rows = getRecords("game");
  const all = [];
  for (const r of rows) {
    let d = {}; try { d = JSON.parse(r.data || "{}"); } catch { continue; }
    if (league && (d.league || "") !== league) continue;
    if (!String(d.date || "").trim() || !String(d.time || "").trim()) continue; // can't judge a game with no date + time
    all.push({
      id: r.id, type: r.type, row: r,
      league: d.league || "", season: r.season == null ? "" : String(r.season),
      date: String(d.date), time: String(d.time), min: _gameMinutes(d.time),
      home: d.home_team || "", away: d.away_team || "", location: String(d.location || ""),
      label: r.name || `#${r.id}`,
    });
  }

  // Field pool per league: every field that league's schedule already uses,
  // in natural order, topped up to fieldsCount with the next numbered fields.
  const want = Math.max(0, Math.floor(Number(fieldsCount) || 0));
  const poolFor = (lg) => {
    const used = [...new Set(all.filter((g) => g.league === lg && g.location).map((g) => g.location))].sort(_naturally);
    const pool = used.slice();
    let n = 0;
    for (const f of pool) { const m = /^Field\s+(\d+)$/i.exec(f); if (m) n = Math.max(n, +m[1]); }
    while (pool.length < want) { n++; const name = `Field ${n}`; if (!pool.includes(name)) pool.push(name); }
    return pool;
  };

  // Group by league + season + date — one game day at a time.
  const days = new Map();
  for (const g of all) {
    const k = `${g.league} ${g.season} ${g.date}`;
  ...
  for (const [, dayGames] of days) {
    const pool = poolFor(dayGames[0].league);
    if (!pool.length) continue;
    // A division's turn comes from when it starts: earliest first, so the division
    // already on the low-numbered fields keeps them and the later one moves up.
    const divStart = new Map();
    for (const g of dayGames) {
      const dv = _divisionOfTeam(g.home) || _divisionOfTeam(g.away) || "";
      g.division = dv;
      const cur = divStart.get(dv);
      if (cur == null || (g.min != null && g.min < cur)) divStart.set(dv, g.min == null ? cur : g.min);
    }
    const ordered = dayGames.slice().sort((a, b) =>
      ((divStart.get(a.division) ?? 1e9) - (divStart.get(b.division) ?? 1e9)) ||
      _naturally(a.division, b.division) ||
      ((a.min ?? 1e9) - (b.min ?? 1e9)) ||
      (pool.indexOf(a.location) - pool.indexOf(b.location)) ||
      (a.id - b.id));

    const claimed = new Map(); // time -> Set of fields taken at that time
    const dayMoves = [];
    const dayStuck = [];
    for (const g of ordered) {
      const busy = claimed.get(g.time) || new Set();
      let target = g.location && !busy.has(g.location) ? g.location : null;
      if (!target) target = pool.find((f) => !busy.has(f)) || null;
      if (!target) {
        // Every field is taken at this hour — there is no clash-free lay-out to be had.
        dayStuck.push({ id: g.id, league: g.league, date: g.date, time: g.time, location: g.location });
        shortfalls.push({ league: g.league, date: g.date, time: g.time, needed: busy.size + 1, available: pool.length });
        target = g.location;
      }
      busy.add(target);
      claimed.set(g.time, busy);
      if (target !== g.location) dayMoves.push({ g, target });
    }
    // All or nothing per game day: shuffling a few games on a day that still ends up
    // double-booked just moves the problem around, so that day is left alone and
    // reported as short of fields instead.
    if (dayStuck.length) { unresolved.push(...dayStuck); continue; }
    for (const { g, target } of dayMoves) {
      if (isSeasonOwned(g.type) && assertWritable(g.row.season)) { locked.push({ id: g.id, season: g.row.season }); continue; }
      changes.push({
        id: g.id, type: g.type,
        name: `${g.date} ${g.time} — ${g.home} vs ${g.away}`,
        before: g.location || "(no field)",
        after: target,
      });
      if (dryRun) continue;
      updateRecord(g.id, { location: target }, actor, `moved to ${target} — ${g.location || "no field"} was double-booked at ${g.time}`);
    }
  }

  // One line per league + hour that ran out of fields, deduplicated.
  const seen = new Set();
  const shortOf = [];
  for (const s of shortfalls) {
    const k = `${s.league}|${s.date}|${s.time}`;
    if (seen.has(k)) continue;
    seen.add(k);
    const peak = shortfalls.filter((x) => `${x.league}|${x.date}|${x.time}` === k).reduce((n, x) => Math.max(n, x.needed), 0);
    shortOf.push({ ...s, needed: peak });
  }
  ...
  return {
    ...
    short_of_fields: shortOf.slice(0, 20),
  };
}
...
export function generateSchedule(league = null, opts = {}) {
  ...
  const games = [];
  // Divisions share the day, so they share one field ledger: the first division
  // to play an hour takes Field 1 up, the next one starts where that left off
  // (Ages 7-8 on Fields 1-2 at 1:00, Ages 9-10 on 3-5), and a division only gets
  // pushed to a later time when the whole field list is genuinely full. Earliest
  // start goes first so the low-numbered fields fill from the front of the day.
  const placeOrder = divisionPlan
    .map((p, i) => ({ p, i, m: _minutesOf(p.start_time) }))
    .sort((a, b) => (a.m == null ? 1e9 : a.m) - (b.m == null ? 1e9 : b.m) || a.i - b.i)
    .map((x) => x.p);
  for (let i = 0; i < totalWeeks; i++) {
    const date = startISO ? weekDate(startISO, i, blackoutSet) : "";
    const dayFields = new Map(); // field -> booked minute ranges, shared by every division that date
    for (const plan of placeOrder) {
      const wkArr = plan.weeksMat[i] || [];
      const wkGames = wkArr.map((g) => ({ week: i + 1, home: g.home, away: g.away }));
      const placed = placeOnFields(wkGames, fields, plan.start_time, gap, dayFields);
...
export function movePlayer(id, changes) {
  ...
  // Putting someone in a bracket their age doesn't fit is an OVERRIDE, and it
  // gets recorded as one so the automatic sort leaves them there. Moving them
  // back into a bracket that does fit clears it.
  let overrideChange = null;
  if (Object.prototype.hasOwnProperty.call(changes, "division") && after.division) {
    const fits = ageFitsDivision(after.division, { ...data, ...after });
    if (fits === false) overrideChange = true;
    else if (fits === true) overrideChange = false;
  }
  if (Object.prototype.hasOwnProperty.call(changes, "division_override")) {
    overrideChange = !!changes.division_override;      // set explicitly
  }
  ...
  if (overrideChange !== null && !!data.division_override !== overrideChange) {
    updates.division_override = overrideChange;
    said.push(overrideChange
      ? `age override on — ${after.division} doesn't cover age ${data.age}, so the automatic sort will leave them there`
      : "age override off — their age fits this bracket now");
  }
...
// Add ONE game to a saved schedule — a make-up, a tournament fixture, a matchup the
// round-robin never produced. Unlike the Build tab (saveSchedule deletes a league's
// games and rewrites them), this writes a single record and leaves the rest alone.
// Called by S-Dot's add_game tool and by the Add a game card on the Schedule page,
// so the signature stays as it was.
export function addGame({ league, date, time, field = "", home_team, away_team, week = null, referee = "" } = {}) {
  if (!league) return { error: "Which league?" };
  const home = tidyText(home_team) || "";
  const away = tidyText(away_team) || "";
  if (!home || !away) return { error: "Both teams are needed." };
  if (home === away) return { error: "A team can't play itself." };
  const iso = _isoDate(date);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(iso)) return { error: "Invalid date (use YYYY-MM-DD)." };
  if (blackoutDateSet(league).has(iso)) return { error: `${iso} is blacked out for ${league}.` };
  // A team's bracket comes from who is ON it, not from its name — the same rule the
  // cross-division prune uses, so a team called "Team 7" is judged correctly too.
  const dvMap = teamDivisionMap(league);
  const dvOf = (t) => (dvMap.get(t) || {}).division || _divisionOfTeam(t);
  const hd = dvOf(home), ad = dvOf(away);
  if (hd && ad && hd !== ad) return { error: `${home} plays ${hd} and ${away} plays ${ad} — teams only play inside their own age bracket.` };

  const existing = getSchedule(league);
  // Week number: match whatever the rest of that date already uses, so the game files
  // under the right week; otherwise continue the season past the last week that has games.
  let wk = week == null || week === "" ? 0 : Number(week) || 0;
  if (!wk) {
    const sameDay = existing.find((g) => g.date === iso && g.week);
    wk = sameDay ? sameDay.week : Math.max(0, ...existing.map((g) => Number(g.week) || 0)) + 1;
  }
  const tm = tidyText(time) || "";
  const loc = tidyText(field) || "";
  const ref = tidyText(referee) || "";
  const data = {
    league, date: iso, time: tm, location: loc,
    home_team: home, away_team: away, referee: ref, week: wk,
  };
  // Season comes from the request scope via applyCreateRecord — same as every other
  // new record, so a game can never be born into the wrong season or a locked one.
  const res = applyCreateRecord("game", `Week ${wk}: ${home} vs ${away}`, data, "user");
  if (res.error) return res;

  // Double-bookings are reported, not blocked — someone adding a make-up game often
  // knows about the clash and wants to sort the slot out afterwards.
  const warnings = [];
  if (tm) for (const g of existing) {
    if (g.date !== iso || (g.time || "") !== tm) continue;
    if (loc && (g.location || "") === loc) warnings.push(`${loc} already has ${g.home} vs ${g.away} at ${tm}.`);
    if (g.home === home || g.away === home) warnings.push(`${home} is already playing at ${tm} that day.`);
    if (g.home === away || g.away === away) warnings.push(`${away} is already playing at ${tm} that day.`);
    if (ref && (g.referee || "") === ref) warnings.push(`${ref} is already assigned to a game at ${tm} that day.`);
  }
  return { ok: true, id: res.id, week: wk, date: iso, home, away, game: data, warnings: [...new Set(warnings)] };
}

...
### Playing up or down — an age override

A league's brackets don't always have somewhere for everyone. Two 13-year-olds
move to a league whose oldest bracket is Ages 10-12: there is nowhere else for
them, so they play down, on purpose.

Just move them. move_player with the league AND the division:

  move_player({id, league: "Sunday Upper Merion", division: "Ages 10-12"})

If the bracket's range doesn't cover their age, the move records an AGE
OVERRIDE by itself and says so in the move's "interpreted" list. Pass that on — "moved
Noah to Sunday Upper Merion, Ages 10-12; that's outside the 10-12 range for a
13-year-old so I've set an age override, which means Re-sort into brackets
won't move him back."

That is the whole point of the flag: sort_into_divisions skips overridden
players and reports how many it left alone. Without it, the next re-sort would
quietly undo the decision.

set_age_override({id, on:false}) removes it, and they sort by age again.
Moving them into a bracket that does fit their age clears it too.

If the league genuinely has no bracket for them, say so and offer to create one
(create_division) rather than forcing an override — an override is for a
deliberate exception, not for a missing bracket.
...
const TOOL_SCHEMAS = [
  ...
  fn("fix_field_clashes", "Repair a SAVED schedule where two divisions sit on one field at one time (schedules built before divisions shared a field list). Moves only the field — dates, times, matchups, referees and scores are untouched; the earliest-starting division keeps its fields and later ones move up. Pass dry:true first to preview. fields_count raises the pool past the fields already in use (say 5 when the schedule only ever used 3). A game day it can't fully separate is left alone and reported in short_of_fields.", {
    league: "string", fields_count: "integer", dry: "boolean",
  }),
  ...
  fn("set_age_override", "Let one player stay in a division their age doesn't fit — playing up or down on purpose. The automatic sort will leave them there. Pass on:false to remove the override so they sort by age again.", {
    id: ["integer", true], on: "boolean", reason: "string",
  }),
...
const DISPATCH = {
  ...
  fix_field_clashes: (a) => T.fixFieldClashes({ dryRun: !!a.dry, league: a.league || null, fieldsCount: a.fields_count || null, actor: "s-dot(cleanup)" }),
  ...
  set_age_override: (a) => T.setDivisionOverride(a.id, a.on !== false, a.reason || ""),
...
function describe(name, a) {
  ...
  switch (name) {
    ...
    case "sort_into_divisions": return "Re-sort every player into the age bracket their age falls into (players with an age override are left alone)";
    case "set_age_override": return a.on === false
      ? `Remove the age override on #${a.id} — they'll sort by age again`
      : `Age override for #${a.id} — keep them in their current division even though their age doesn't fit`;

...
function ListTab({ type, fields, L, records, reload, refresh, setFlash, playerNames, divisions = [], sortIntoDivisions, sortingDivs = false }) {
  ...
  // Season scope — follows the sidebar's global season picker (localStorage
  // ff_season). Changing the picker remounts this component, so reading it once
  // per mount stays in sync. Untagged records show under every season.
  const seasonSel = (() => {
    try { return (typeof localStorage !== "undefined" && localStorage.getItem("ff_season")) || ""; } catch { return ""; }
  })();
  // Which leagues run in the selected season (Start-a-season mapping). Null =
  // no mapping recorded = show every league (back-compat).
  const [seasonLeagueMap, setSeasonLeagueMap] = useState(null);
  useEffect(() => {
    if (type !== "player") { setSeasonLeagueMap(null); return; }
    let dead = false;
    (async () => {
      try { const s = await api.seasonsList(); if (!dead && !s.error) setSeasonLeagueMap(s.leaguesBySeason || {}); } catch {}
    })();
    return () => { dead = true; };
  }, [type]);
  ...
  // Scope the league pills to the selected season's leagues, when mapped.
  if (type === "player" && seasonSel && seasonLeagueMap) {
    const snLg = seasonLeagueMap[seasonSel];
    if (Array.isArray(snLg) && snLg.length) leagueOpts = leagueOpts.filter((l) => snLg.includes(l));
  }
    ...
    <div>
      ...
      <div className="between" style={{ margin: "8px 0", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        <div className="small" style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          ...
          {type === "player" && seasonSel && (
            <span className="chip" title="Set by the season picker in the sidebar">{seasonSel}</span>
          )}
